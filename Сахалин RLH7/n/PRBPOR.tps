...
begin
  ...
  else if Trunc(@M1.PRBPOR/100) = 2028 then
    ResultValue := '2028'	{ берёза кам }
  else if Trunc(@M1.PRBPOR/100) = 3028 then
    ResultValue := '3020'	{ береза }
  else if Trunc(@M1.PRBPOR/10) = 30423 then
    ResultValue := '3040'	{ осина }
  else if Trunc(@M1.PRBPOR/100) = 3024 then
    ResultValue := '3140'	{ ольха }
  else if Trunc(@M1.PRBPOR/10) = 30420 then
    ResultValue := '3240'	{ тополь }
  else if Trunc(@M1.PRBPOR/100) = 3043 then
    ResultValue := '3044'	{ ива }
  else if Trunc(@M1.PRBPOR) = 700510 then
    ResultValue := '700'	{ кедровый стланик }
  else
    ResultValue := '0';
  ...
  if (@M1.KATZEM >= 1100) and (@M1.KATZEM < 1108) then
    begin			{ покр. лесом }
      ...
      ResultValue := ResultValue { культуры несомкн. }
    ...
    begin			{ культуры по группам возраста }
      ...
      ResultValue := ResultValue 	{ культ. реконстр. }
